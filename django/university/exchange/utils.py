...
def create_marketplace_exchange_on_db(exchanges, curr_student):
    marketplace_exchange = MarketplaceExchange.objects.create(
        issuer=curr_student, accepted=False
    )
    for exchange in exchanges:
        ...
        MarketplaceExchangeClass.objects.create(
            ...
            course_unit_id=exchange["course_unit_id"],
            course_unit_name=exchange["course_unit"],
...
def build_marketplace_submission_schedule(schedule, submission, auth_student):
    for exchange in submission:
        ...
        auth_user_valid = (class_auth_student_goes_from, course_unit) in schedule[
            auth_student
        ]
        if not (auth_user_valid):
            return (ExchangeStatus.STUDENTS_NOT_ENROLLED, None)

        schedule[auth_student][(class_auth_student_goes_to, course_unit)] = (
            SigarraController()
            .get_class_schedule(
                schedule[auth_student][(class_auth_student_goes_from, course_unit)][
                    "ocorrencia_id"
                ],
                class_auth_student_goes_to,
            )
            .data[0][0]
        )  # get class schedule
        del schedule[auth_student][
            (class_auth_student_goes_from, course_unit)
        ]  # remove old class of other student
...
def build_new_schedules(student_schedules, exchanges, auth_username):
    for curr_exchange in exchanges:
        ...
        class_other_student_goes_to = curr_exchange[
            "classNameRequesterGoesFrom"
        ]  # The other student goes to its new class
        # If participant is neither enrolled in that course unit or in that class
        other_student_valid = (
            class_auth_student_goes_to,
            course_unit,
        ) in student_schedules[other_student]
        auth_user_valid = (
            class_other_student_goes_to,
            course_unit,
        ) in student_schedules[auth_username]
        ...
        (
            student_schedules[auth_username][user_uc],
            student_schedules[other_student][other_user_uc],
        ) = (
            student_schedules[other_student][user_uc],
...
def build_student_schedule_dict(schedule: list, from_local_db: bool = False):
    res = {}

    for item in schedule:
        print("CURRENT ITEM: ", item)
        for slot in item["classInfo"]["slots"]:
            if slot["lesson_type"] == "TP" or slot["lesson_type"] == "PL":
                res[
                    (item["classInfo"]["name"], item["courseInfo"]["id"])
                ] = item

    return res

def check_class_schedule_overlap(
    day_1: int, start_1: int, end_1: int, day_2: int, start_2: int, end_2: int
) -> bool:
...
def exchange_overlap(student_schedules, username) -> bool:
    for key, class_schedule in student_schedules[username].items():
        for other_key, other_class_schedule in student_schedules[username].items():
            ...
            (
                class_schedule_day,
                class_schedule_start,
                class_schedule_end,
                class_schedule_type,
            ) = (
                class_schedule["dia"],
                class_schedule["hora_inicio"] / 3600,
                class_schedule["aula_duracao"] + class_schedule["hora_inicio"] / 3600,
                class_schedule["tipo"],
            )
            (
                overlap_param_day,
                ...
                other_class_schedule["dia"],
                other_class_schedule["hora_inicio"] / 3600,
                other_class_schedule["aula_duracao"]
                + other_class_schedule["hora_inicio"] / 3600,
                other_class_schedule["tipo"],
            )

            if (
                check_class_mandatory(class_schedule_type)
                and check_class_mandatory(overlap_param_type)
                and check_class_schedule_overlap(
                    class_schedule_day,
                    class_schedule_start,
                    class_schedule_end,
                    overlap_param_day,
    ...
    for schedule in schedule_data:
        ...
        for docente in schedule["docentes"]:
            ...
            if len(professor) < 1:
                continue
            professors.append(
                {"name": docente["doc_nome"], "acronym": professor[0].professor_acronym}
            )
